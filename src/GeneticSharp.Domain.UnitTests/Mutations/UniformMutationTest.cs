...
namespace GeneticSharp.Domain.UnitTests
{
	...
	public class UniformMutationTest
	{
		...
		public void Mutate_NoIndexes_RandomOneIndex ()
		{
            ...
            var chromosome = MockRepository.GenerateStub<ChromosomeBase>();
			chromosome.AddGenes (new List<Gene>() 
			{ 
				new Gene() { Value = 1 },
				new Gene() { Value = 1 },
				new Gene() { Value = 1 } 
		...
		public void Mutate_InvalidIndexes_Exception ()
		{
			...
			ExceptionAssert.IsThrowing (new ArgumentOutOfRangeException ("chromosome", "The chromosome has no gene on index 3. The chromosome genes lenght is 3."), () => {
				target.Mutate (chromosome);
			});
