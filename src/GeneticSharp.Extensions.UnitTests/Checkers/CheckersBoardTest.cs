...
namespace GeneticSharp.Extensions.UnitTests.Checkers
{
	...
	public class CheckersBoardTest
	{
        ...
        [Test()]
        public void CountCapturableByPiece_Null_Exception()
        {
            var target = new CheckersBoard(8);

            ExceptionAssert.IsThrowing(new ArgumentNullException("piece"), () =>
            {
                target.CountCapturableByPiece(null);
            });
        }

        [Test()]
        public void CountCapturableByPiece_ThereIsNoEnemyPieceAround_Zero()
        {
            var target = new CheckersBoard(8);

            foreach (var p in target.PlayerOnePieces)
            {
                Assert.AreEqual(0, target.CountCapturableByPiece(p));
            }
